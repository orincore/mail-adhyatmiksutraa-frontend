...
export default function WebinarDetailPage() {
  ...
  const [sendNowReminder, setSendNowReminder] = useState<Reminder | null>(null);
  const [sendNowPreview, setSendNowPreview] = useState<{ email_pending: number; whatsapp_pending: number } | null>(null);
  const [loadingSendNowPreview, setLoadingSendNowPreview] = useState(false);
  ...
  async function openSendNow(reminder: Reminder) {
    setSendNowReminder(reminder);
    setSendNowPreview(null);
    setLoadingSendNowPreview(true);
    try {
      const res = await fetch(`/api/webinars/${webinarId}/reminders/${reminder.id}/send-now-preview`);
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "Failed to load audience count");
      }
      setSendNowPreview(await res.json());
    } catch (err: any) {
      showNotification(err.message || "Failed to load audience count", "error");
      setSendNowReminder(null);
    } finally {
      setLoadingSendNowPreview(false);
    }
  }

  async function handleSendNow() {
    if (!sendNowReminder) return;
    setSendingNow(true);
    try {
      const res = await fetch(`/api/webinars/${webinarId}/reminders/${sendNowReminder.id}/send-now`, {
        method: "POST",
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "Failed to send instantly");
      }
      showNotification(`"${sendNowReminder.name}" sent instantly`);
      setSendNowReminder(null);
      setSendNowPreview(null);
      loadDetail();
    } catch (err: any) {
      showNotification(err.message || "Failed to send instantly", "error");
    } finally {
      setSendingNow(false);
    }
  }

  // Reactivating is low-risk (no notification goes out) so it stays a single
  // click. Cancelling is the risky direction — it can blast a WhatsApp
  // notice to every registrant — so that path opens a confirmation modal
  // (below) with a "notify registrants" toggle instead of firing immediately.
  async function toggleWebinarCancelled() {
    if (!webinar || cancelling) return;
    if (webinar.status !== "cancelled") {
      setNotifyOnCancel(true);
      setShowCancelModal(true);
      return;
    }
    await updateWebinarStatus("upcoming", true);
  }

  async function updateWebinarStatus(newStatus: "upcoming" | "cancelled", notify: boolean) {
    if (!webinar || cancelling) return;
    setCancelling(true);
    try {
      ...
      showNotification(
        newStatus === "cancelled"
          ? notify
            ? "Webinar cancelled — registrants notified, pending reminders skipped"
            : "Webinar cancelled silently — no notifications sent, pending reminders skipped"
          : "Webinar reactivated"
      );
  ...
  // The Play/Pause button only shows what clicking it *does* (its icon is the
  // next action, not the current state), which reads backwards at a glance —
  // a Play icon on a paused reminder looks like it's currently running. This
  // badge states the reminder's actual status in words so there's no ambiguity.
  const reminderStatusBadge = (status: Reminder["status"]) => {
    const styles: Record<string, string> = {
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          {webinar.email_eligible_count < webinar.registrant_count && (
            <p
              className="text-xs text-amber-600 inline-flex items-center gap-1 mt-1"
              title="These registrants previously unsubscribed, bounced, or reported a past email as spam — email reminders skip them by design. They still receive WhatsApp reminders if they have a number on file."
            >
              <AlertCircle className="h-3.5 w-3.5" />
              {webinar.registrant_count - webinar.email_eligible_count} won't receive email reminders (previously unsubscribed/bounced)
            </p>
          )}
        </div>
        {canWrite && (
          <button
            ...
            disabled={cancelling}
            className={`inline-flex items-center gap-1.5 px-4 py-2 font-semibold rounded-xl text-sm transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
              webinar.status === "cancelled"
          ...
          >
            {webinar.status === "cancelled" ? (
              <>
                <RotateCcw className="h-4 w-4" /> {cancelling ? "Reactivating..." : "Reactivate"}
              </>
            ) : (
              <>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900 inline-flex items-center gap-2">
            <Bell className="h-4.5 w-4.5 text-violet-600" /> Reminders
          </h2>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {PRESETS.map((p) => (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 hover:border-violet-300 hover:bg-violet-50 rounded-lg text-xs font-semibold text-slate-700 transition-all cursor-pointer"
              >
              ...
              return (
                <div key={r.id} className="py-3 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {(r.stats.sent > 0 || r.stats.whatsapp_sent > 0) && (
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        {r.stats.sent > 0 &&
                          `${r.stats.sent} emailed${r.stats.opens ? ` · ${r.stats.opens} opened` : ""}${
                            r.stats.clicks ? ` · ${r.stats.clicks} clicked` : ""
                          }`}
                        {r.stats.sent > 0 && r.stats.whatsapp_sent > 0 && " · "}
                        {r.stats.whatsapp_sent > 0 &&
                          `${r.stats.whatsapp_sent} WhatsApp sent${
                            r.stats.whatsapp_failed ? ` (${r.stats.whatsapp_failed} failed)` : ""
                          }`}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {canWrite && showWhatsapp && (
                      <button
                        ...
                        className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg border border-violet-200 text-violet-700 hover:bg-violet-50 text-xs font-semibold cursor-pointer"
                    ...
                    {canWrite && (
                      <button
                        onClick={() => openSendNow(r)}
                        disabled={busy}
                        className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg border border-amber-200 text-amber-700 hover:bg-amber-50 disabled:opacity-40 text-xs font-semibold cursor-pointer"
                        title="Send this reminder to everyone right now, regardless of its scheduled time"
                      >
                        <Zap className="h-3.5 w-3.5" /> Send Instantly
                      </button>
                    )}
                    {canWrite && (
                      <button
      ...
      {registrants.length > 0 && (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-3">
          <h2 className="text-lg font-bold text-gray-900 inline-flex items-center gap-2">
            <Users className="h-4.5 w-4.5 text-violet-600" /> Registrants ({registrants.length})
          </h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-md w-full overflow-hidden animate-fadeIn text-left max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateReminder} className="p-6 space-y-4">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-2.5 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-all cursor-pointer"
              >
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-sm w-full overflow-hidden animate-fadeIn text-left">
            ...
            <form onSubmit={handleSendWhatsappTest} className="p-6 space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-all cursor-pointer inline-flex items-center justify-center gap-1.5"
              >
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-sm w-full overflow-hidden animate-fadeIn text-left">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 text-base flex items-center gap-1.5">
                <Zap className="h-4 w-4 text-amber-500" /> Send Instantly
              </h3>
              <button
                onClick={() => {
                  setSendNowReminder(null);
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-slate-500">
                This sends "<span className="font-semibold text-slate-700">{sendNowReminder.name}</span>" right now,
                skipping its scheduled time — it goes to everyone currently registered who hasn't already received
                it (a large audience sends its first batch now, the rest on the next regular sweep, same as any
                other reminder).
              </p>
              {loadingSendNowPreview ? (
                <div className="flex items-center justify-center py-6 text-slate-400 text-sm gap-2">
                  <div className="h-4 w-4 border-2 border-slate-200 border-t-amber-500 rounded-full animate-spin" />
                  Counting recipients…
                </div>
              ) : sendNowPreview ? (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-1.5">
                  {sendNowReminder.channel !== "whatsapp" && (
                    <p className="text-sm text-amber-800 flex items-center gap-1.5">
                      <Mail className="h-3.5 w-3.5" />
                      Email will be sent to{" "}
                      <span className="font-bold">{sendNowPreview.email_pending}</span> participant
                      {sendNowPreview.email_pending === 1 ? "" : "s"}
                    </p>
                  )}
                  {sendNowReminder.channel !== "email" && (
                    <p className="text-sm text-amber-800 flex items-center gap-1.5">
                      <WhatsAppIcon className="h-3.5 w-3.5" />
                      WhatsApp message will be sent to{" "}
                      <span className="font-bold">{sendNowPreview.whatsapp_pending}</span> participant
                      {sendNowPreview.whatsapp_pending === 1 ? "" : "s"}
                    </p>
                  )}
                  <p className="text-xs text-amber-700 pt-1">Do you want to continue?</p>
                </div>
              ) : null}
              <div className="flex gap-2">
                <button
                  ...
                  disabled={sendingNow}
                  className="flex-1 py-2.5 border border-slate-200 hover:bg-slate-50 disabled:opacity-50 text-slate-600 font-semibold rounded-xl text-sm transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSendNow}
                  disabled={sendingNow || loadingSendNowPreview || !sendNowPreview}
                  className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-all cursor-pointer inline-flex items-center justify-center gap-1.5"
                >
                  <Zap className="h-3.5 w-3.5" />
                  {sendingNow ? "Sending..." : "Continue"}
                </button>
              </div>
            </div>
          </div>
      ...
      {showCancelModal && webinar && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[99999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-sm w-full overflow-hidden animate-fadeIn text-left">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 text-base flex items-center gap-1.5">
                <Ban className="h-4 w-4 text-rose-600" /> Cancel Webinar
              </h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-slate-500">
                This cancels "<span className="font-semibold text-slate-700">{webinar.title}</span>" and skips every
                pending reminder for it. {webinar.registrant_count} registrant{webinar.registrant_count === 1 ? "" : "s"}{" "}
                {webinar.registrant_count === 1 ? "is" : "are"} registered for this occurrence.
              </p>
              <label className="flex items-start gap-2.5 bg-slate-50 border border-slate-200 rounded-xl p-3 cursor-pointer">
                ...
                <span className="text-sm text-slate-700">
                  <span className="font-semibold">Notify registrants via WhatsApp</span>
                  <span className="block text-xs text-slate-400 mt-0.5">
                    {notifyOnCancel
                      ? "Sends the cancellation notice once to everyone registered."
                      : "Cancels silently — no message goes out to registrants."}
                  </span>
                </span>
              </label>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => updateWebinarStatus("cancelled", notifyOnCancel)}
                  disabled={cancelling}
                  className="flex-1 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-all cursor-pointer inline-flex items-center justify-center gap-1.5"
                >
