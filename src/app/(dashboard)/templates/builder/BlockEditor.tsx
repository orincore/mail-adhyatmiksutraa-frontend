  ...
  return (
    <div className="relative shrink-0">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10.5px] font-medium text-violet-700 bg-violet-50 hover:bg-violet-100 transition-colors cursor-pointer"
      >
...
const inputCls =
  "w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-[12.5px] focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 bg-white text-slate-900";
      ...
      return (
        <div className="space-y-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 border border-dashed border-slate-300 hover:border-violet-400 hover:text-violet-600 text-slate-500 font-medium rounded-lg text-[11.5px] transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-4 gap-2">
          {BLOCK_PALETTE.map((p) => (
            <button
              ...
              className="flex flex-col items-center justify-center gap-1 py-2.5 rounded-xl border border-slate-200 hover:border-violet-300 hover:bg-violet-50 text-slate-600 hover:text-violet-700 transition-all cursor-pointer"
            >
        ...
        {design.blocks.length === 0 ? (
          ...
          design.blocks.map((block, idx) => {
            ...
            return (
              <div
                key={block.id}
                className={`rounded-2xl border bg-white transition-all ${selected ? "border-violet-400 ring-2 ring-violet-500/10" : "border-slate-200"}`}
              >
                <div
                  role="button"
                  tabIndex={0}
