...
export default async function CampaignAnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!analytics) {
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
        ...
        <Link href="/campaigns" className="text-violet-600 hover:underline text-sm mt-2 inline-block">
          Back to Campaigns
        </Link>
      </div>
    );
  ...
  const statCards = [
    ...
    { label: "Unique Opens", value: totals.uniqueOpens, sub: `${openRate.toFixed(1)}%`, icon: Mail, color: "bg-violet-50 text-violet-600" },
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex items-center gap-3 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <span className={`ml-auto px-3 py-1 rounded-full text-[10px] font-bold uppercase border shrink-0 ${
          campaign.status === "sent" ? "bg-violet-50 text-violet-600 border-violet-100" :
              ...
              <div key={v.variant} className="border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-violet-50 rounded-lg p-2 text-center">
                    <div className="font-bold text-violet-700">{(v.openRate * 100).toFixed(1)}%</div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="flex items-end gap-1.5 h-32 overflow-x-auto pb-1">
            {timeline.map((t: any) => (
              <div key={t.label} className="flex items-end gap-0.5 shrink-0 h-full" title={`${t.label} — Sent: ${t.sent}, Opens: ${t.opens}, Clicks: ${t.clicks}`}>
                ...
                />
                <div
                  className="w-2 bg-violet-400 hover:bg-violet-500 rounded-sm transition-colors"
        ...
        <div className="flex gap-4 mt-3 text-[10px] font-bold text-slate-500">
          ...
          <div className="flex items-center gap-1.5"><span className="h-1.5 w-3 rounded-sm bg-violet-400 block" /> Opens</div>
