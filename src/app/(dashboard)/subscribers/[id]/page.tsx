  ...
  if (!data?.subscriber) {
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
        ...
        <Link href="/subscribers" className="text-violet-600 hover:underline text-sm mt-2 inline-block">
          Back to Subscribers
        </Link>
      </div>
    );
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex items-center gap-3 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <span className={`ml-auto px-3 py-1 rounded-full text-[10px] font-bold uppercase border shrink-0 ${
          sub.status === "subscribed" ? "bg-violet-50 text-violet-600 border-violet-100" :
