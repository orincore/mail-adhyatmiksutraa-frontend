  ...
  return (
    <RoleProvider role={userRole}>
      <div className="flex h-screen bg-[#f3f5f0] overflow-hidden font-sans p-4 gap-5">
        {/* Sidebar Navigation */}
        <aside className="w-[220px] bg-white flex flex-col pt-4 pb-5 rounded-3xl border border-slate-200/70 shadow-surface shrink-0">
          <div className="flex items-center gap-2.5 px-3 mb-7 shrink-0">
            <div className="h-11 w-11 rounded-2xl overflow-hidden flex items-center justify-center shrink-0 bg-slate-50 border border-slate-100 p-1.5">
              <img src={BRAND_LOGO_URL} alt={`${BRAND_NAME} Logo`} className="max-h-full max-w-full object-contain" />
            </div>
            <span className="text-[14px] font-semibold text-slate-800 truncate">{BRAND_NAME}</span>
          </div>
          <SidebarNav />
          <div className="mt-auto pt-4 px-3 flex items-center gap-2.5">
            <div className="h-9 w-9 rounded-full bg-slate-900 text-white text-[11px] font-semibold flex items-center justify-center select-none shrink-0">
              {initials || "A"}
            </div>
            <div className="min-w-0">
              <p className="text-[12.5px] font-semibold text-slate-800 truncate">{adminName}</p>
              <p className="text-[11px] text-slate-400 capitalize truncate">{userRole}</p>
            </div>
          </div>
        </aside>
